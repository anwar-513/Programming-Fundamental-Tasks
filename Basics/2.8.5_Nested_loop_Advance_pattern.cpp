...
char uppercaseConversion(char ch);
...
//	pattern16(number);
	char ch;
	cout<<"Enter the character for pattern---> ";
	cin>>ch;
	
	while(ch < 65 && ch > 90 && ch < 97 && ch > 122)
	{
		cout<<"\n the entered value is not a character!!..";
		cout<<"\n Enter a Character---> ;";
		cin>>ch;
	}
...
void pattern17(char ch)
{
	uppercaseConversion(ch);
	for(char i = 'A'; i <= ch; i++)
	{
		for(char j = 'A'; j <= i; j++)
		{
